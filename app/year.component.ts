...
export class YearComponent {
  ...
  getTotalPoints(){
    if (this.year.courses.filter(e=>!e.dispensation).length > 0)
      return Math.round(this.year.courses.filter(e=>!e.dispensation).map(e=>e.studypoints).reduce((t,e)=>t+e));
    return 0;
  }
  getTotalPointsOfP(p :number){
    if (this.year.courses.filter(e => ((e.start)<=p)&&((e.start+e.duration)>p)&&!e.dispensation).length > 0)
      return Math.round( this.year.courses.filter(e => ((e.start)<=p)&&((e.start+e.duration)>p)&&!e.dispensation).map(e=>e.studypoints/e.duration).reduce((t,e)=>t+e));
    return 0;
  }
}
